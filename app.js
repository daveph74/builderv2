class DragDropEditor {
    ...
    updateZoomUI() {
        ...
        canvasInfo.textContent = `Canvas: ${this.canvas.offsetWidth} x ${this.canvas.offsetHeight} px - ${Math.round(this.zoomLevel * 100)}%`;
    }
    ...
    zoomToFit() {
        ...
        const scaleX = availableWidth / this.canvas.offsetWidth;
        const scaleY = availableHeight / this.canvas.offsetHeight;
    ...
    updateRulersForZoom() {
        ...
        const centerX = this.canvas.offsetWidth / 2; // Canvas width / 2
        const centerY = this.canvas.offsetHeight / 2; // Canvas height / 2
    ...
    addEventListenersToElement(element) {
        ...
        element.addEventListener('blur', () => {
            if (element.contentEditable === 'true') {
                ...
                if (elementData) {
                    elementData.text = element.innerHTML;
                }
            }
        });
    }
    ...
    handleElementDrag(e) {
        ...
        if (this.snapEnabled) {
            const snapResult = this.getSnapPosition(x, y, this.selectedElement);
            x = snapResult.x;
            y = snapResult.y;
    ...
    handleGlobalClick(e) {
        ...
        if (
            ...
            !document.getElementById('propertiesPanel').contains(e.target) &&
            !document.getElementById('layersPanel').contains(e.target) &&
    ...
    handleResize(e) {
        ...
        // Constrain to canvas bounds
        newX = Math.max(0, Math.min(newX, this.canvas.offsetWidth - newWidth));
        newY = Math.max(0, Math.min(newY, this.canvas.offsetHeight - newHeight));
    ...
    stopRotation() {
        ...
        document.removeEventListener('mouseup', this.stopRotation.bind(this));
        document.body.style.userSelect = '';
    }
    ...
    duplicateElement(element) {
        ...
        if (elementData) {
            ...
            newElementData.colorCmyk = elementData.colorCmyk ? { ...elementData.colorCmyk } : this.hexToCmyk(elementData.color);
            newElementData.backgroundColor = elementData.backgroundColor;
            newElementData.backgroundColorCmyk = elementData.backgroundColorCmyk ? { ...elementData.backgroundColorCmyk } : this.hexToCmyk(elementData.backgroundColor || '#3498db');
            newElementData.borderColor = elementData.borderColor;
            newElementData.borderColorCmyk = elementData.borderColorCmyk ? { ...elementData.borderColorCmyk } : this.hexToCmyk(elementData.borderColor || '#2c3e50');
            ...
            if (newVisualElement) {
                ...
                if (elementData.text) {
                    newVisualElement.innerHTML = elementData.text;
                }
        ...
        if (elementData.type === 'text' || elementData.type === 'heading') {
            specificProperties = `
                ...
                <div class="property-group">
                    ...
                    <input type="text" id="prop-tailwindClasses" value="${elementData.tailwindClasses || ''}" placeholder="bg-blue-500 text-white p-2 rounded">
                    <small class="help-text">Add Tailwind CSS classes (e.g., bg-blue-500 text-white p-2 rounded)</small>
                </div>
            `;
        }
    ...
    switchColorMode(mode, elementData, colorType = 'text') {
        switch (colorType) {
            case 'text': elementData.colorMode = mode; break;
            case 'background': elementData.backgroundColorMode = mode; break;
            case 'border': elementData.borderColorMode = mode; break;
        }
        this.updatePropertiesPanel();
    }
    ...
    async generatePDF() {
        try {
            ...
            const page = pdfDoc.addPage([this.canvas.offsetWidth, this.canvas.offsetHeight]);
            ...
            for (const elementData of this.elements) {
                ...
                const y = this.canvas.offsetHeight - elementData.y - elementData.height; // Flip Y coordinate for PDF
                ...
                switch (elementData.type) {
                    ...
                    case 'heading':
                        ...
                        const plainText = this.stripHtmlTags(elementData.text);
                    ...
                    case 'image':
                        if (elementData.imageUrl && elementData.imageUrl.trim() !== '') {
                            try {
                                ...
                                const imageResponse = await fetch(elementData.imageUrl);
    ...
    // Strip HTML tags to get plain text for PDF generation
    stripHtmlTags(html) {
    ...
    getSnapPosition(x, y, element) {
        ...
        this.elements.forEach(elementData => {
            if (elementData.id === element.dataset.id) return;
            
            const otherElement = document.querySelector(`[data-id="${elementData.id}"]`);
            if (!otherElement) return;
            
            const otherX = elementData.x;
            const otherY = elementData.y;
            const otherWidth = elementData.width;
            const otherHeight = elementData.height;
    ...
    async saveTemplate() {
        ...
        const width = parseInt(this.canvas.style.width) || parseInt(this.canvas.offsetWidth) || 793;
        const height = parseInt(this.canvas.style.height) || parseInt(this.canvas.offsetHeight) || 1122;
    ...
    applyTemplate(templateData) {
        ...
        templateData.elements.forEach(elementData => {
            const element = this.createElement(elementData.type, elementData.x, elementData.y);
    ...
    enableCKEditorForElement(element, elementData) {
        // Prevent multiple editors
        if (element.querySelector('textarea')) return;
        const currentText = elementData.text || element.innerHTML;
        const textareaId = 'ckeditor-' + elementData.id;
        element.innerHTML = `<textarea id="${textareaId}">${currentText}</textarea>`;
        ...
        CKEDITOR.replace(textareaId, {
            ...
            // No custom toolbar: use default full toolbar
        });
        CKEDITOR.instances[textareaId].on('blur', () => {
            const data = CKEDITOR.instances[textareaId].getData();
            element.innerHTML = data || 'Text';
            elementData.text = data || 'Text';
...
function updateTemplateTabInfo() {
    ...
    if (canvas) {
        ...
        if (canvasSize) {
            const width = parseInt(canvas.style.width) || 793;
            const height = parseInt(canvas.style.height) || 1122;
    ...
    if (templateWidth && templateHeight && canvas) {
        const width = parseInt(canvas.style.width) || parseInt(canvas.offsetWidth) || 793;
        const height = parseInt(canvas.style.height) || parseInt(canvas.offsetHeight) || 1122;
...
function setupTemplateSizeControls() {
    ...
    if (applyTemplateSize && templateWidth && templateHeight && canvas) {
        applyTemplateSize.addEventListener('click', function() {
            ...
            // Sync with main canvas controls if they exist
    ...
    // Sync template size inputs when main canvas controls change
    const canvasWidthInput = document.getElementById('canvasWidthInput');
    const canvasHeightInput = document.getElementById('canvasHeightInput');
    
    if (canvasWidthInput && templateWidth) {
        canvasWidthInput.addEventListener('input', () => {
            templateWidth.value = canvasWidthInput.value;
        });
    }
    
    if (canvasHeightInput && templateHeight) {
        canvasHeightInput.addEventListener('input', () => {
            templateHeight.value = canvasHeightInput.value;
        });
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    // First, initialize canvas with default dimensions
    const widthInput = document.getElementById('canvasWidthInput');
    const heightInput = document.getElementById('canvasHeightInput');
    const canvas = document.getElementById('canvas');
    
    if (widthInput && heightInput && canvas) {
        const defaultWidth = parseInt(widthInput.value, 10);
        const defaultHeight = parseInt(heightInput.value, 10);
    ...
    // Debug functions
    window.createTestGuide = (orientation = 'vertical', position = 100) => {
        return editor.createTestGuide(orientation, position);
    };
    
    window.debugGuides = () => {
        console.log('Current guides:', editor.guideLines);
        console.log('Guide container:', document.querySelector('.guide-lines'));
        console.log('Canvas:', editor.canvas);
    };
    
    // Check if we're on a template-specific URL and auto-load the template
    ...
    // Set up template size controls
    setupTemplateSizeControls();
    
    // Set up canvas resize functionality
    const applyBtn = document.getElementById('applyCanvasSize');
    if (applyBtn && widthInput && heightInput && canvas) {
        applyBtn.addEventListener('click', function() {
            const w = Math.max(100, Math.min(2000, parseInt(widthInput.value, 10)));
            const h = Math.max(100, Math.min(2000, parseInt(heightInput.value, 10)));
            canvas.style.width = w + 'px';
            canvas.style.height = h + 'px';
            // Wait for DOM to update, then redraw rulers
            if (window.editor && typeof window.editor.updateRulersForZoom === 'function') {
                console.log('updating rulers');
                setTimeout(() => {
                    window.editor.updateRulersForZoom();
                }, 0);
            }
            
            // Update template tab info when canvas size changes
            updateTemplateTabInfo();
        });
    }
}); 